'''
Calculates cubs (density and total ESP) and CP of total ESP for given wavefunction data (supports .wfx/.wfn/.fchk)
Then drawing them using Mayavi code

Input information:
python3.8 VisMap3.0.py *file with wfn data* -nproc=Num  -mode=new/old -vis=y/n -CPisov=number
        -nproc = Num - number of processors used in calculation
        ...
        -CPisov = isoval - pre-generation of CPs (in order to calculate ECP CPs before the visualization starts) 

Changelog:
v4.1rc1
repaired the problems with mgrid (switched to np.meshgrid function) which solved problems with Te2Ph2_i2_zora_pbe0_tzvp_en.wfn
...
def VisualizeData(CENTERS,CUBdat,CUBdatESP,xx,yy,zz):
    ...
    #print(len(xx),len(xx[0]),len(xx[0][0]),len(yy),len(yy[0]),len(yy[0][0]),len(zz),len(zz[0]),len(zz[0][0]),len(CUBdat),len(CUBdat[0]),len(CUBdat[0][0]))
    ...
    mlab.colorbar(object=surface, title='ESP, kkal/mol\n', orientation='vertical', nb_labels=3)
...
#print("pointsED:",pointsv1,pointsv2,pointsv3)
...
#print("pointsESP:",pointsv1,pointsv2,pointsv3)
for x in range(pointsv1):
    for y in range(pointsv2):
        for z in range(pointsv3):
